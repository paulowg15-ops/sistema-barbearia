        ...
        with st.container(border=True):
            ...
            if len(st.session_state["carrinho_comanda"]) > 0:
                ...
                if st.button("❌ Remover Item Selecionado", use_container_width=True):
                    ...
                    st.session_state["carrinho_comanda"].pop(index_remover)
                    st.rerun()
                
                st.markdown("---")
                st.subheader("💳 Divisão do Pagamento Misto")
                ...
                if st.button("🚀 Finalizar Conta", type="primary", use_container_width=True, disabled=(saldo_restante != 0)):
                    ...
                    for item_c in st.session_state["carrinho_comanda"]:
                        ...
                        if item_c["Tipo"] == "Produto":
                            produtos_df.loc[produtos_df["Nome do Produto"] == item_c["Item"], "Estoque Atual"] -= int(item_c["Quantidade"])
                    ...
                    st.session_state["carrinho_comanda"] = []
                    st.success("✅ Venda registrada e itens baixados das prateleiras!")
                    time.sleep(1.2)
                    st.rerun()
...
# ---------------- MÓDULO NEW: EXTRATO DE FLUXO SEPARADO ----------------
    ...
    else: st.info("Nenhum serviço ou produto nesse intervalo.")
    ...
    else: st.info("Nenhum pagamento real no caixa nesse intervalo.")
    ...
    else: st.info("Nenhum fechamento diário realizado no período.")
...
# ---------------- MÓDULO 4: CORRIGIR LANÇAMENTOS ----------------
    ...
    with tab_corr_vendas:
        if not vendas_df.empty and "Forma de Pagamento" in vendas_df.columns:
            ...
            with col_ed2:
                with st.container(border=True):
                    ...
                    if st.button("❌ Excluir Lançamento Errado", use_container_width=True):
                        ...
                        texto_rastro = f"Dia lancado: {venda_deletada_info['Data']} | Item: {venda_deletada_info['Item']} | Total: R$ {venda_deletada_info['Valor Total']}"
                        registrar_rastro_auditoria("EXCLUSÃO DE COMANDA", texto_rastro)
    ...
    with tab_corr_fechamentos:
        if not fechamentos_df.empty:
            ...
            if st.button("❌ Excluir Fechamento Selecionado", use_container_width=True):
                ...
                texto_rastro = f"Caixa do Dia: {fech_deletado_info['Data']} | Total Contado: R$ {fech_deletado_info['Total Real']}"
                ...
                st.success("🗑️ Fechamento removido!")
                time.sleep(1.2)
                st.rerun()
...
# ---------------- MÓDULO 5: FECHAMENTO DE DIA ----------------
    ...
    with tab_f_1:
        ...
        vendas_do_dia = vendas_df[vendas_df["Data"] == data_final_fechamento] if not vendas_df.empty and "Data" in vendas_df.columns else pd.DataFrame(columns=["Valor Total"])
            ...
            with st.container(border=True):
                ...
                if st.button("Salvar Produto", type="primary", use_container_width=True):
                    if p_nome != "":
                        ...
                        novo_p = pd.DataFrame([{"ID": novo_id, "Nome do Produto": p_nome, "Preço de Venda": p_venda, "Preço de Custo": p_custo, "Estoque Inicial": p_estoque, "Estoque Atual": p_estoque, "Comissão Barbeiro (R$)": p_comis}])
...
# ---------------- MÓDULO 10: PAINEL DE RELATÓRIOS ----------------
    ...
    total_despesas_fixas = g_filtrados["Valor (R$)"].sum() if not g_filtrados.empty else 0.0
    ...
    with tab_rel2:
        ...
        with col_staff1:
            ...
            if tipo_lancamento_staff == "Produto do Estoque":
                if not produtos_atualizados_df.empty:
                    ...
                    if not linha_prod_ref.empty:
                        ...
                        vendas_totais_item = vendas_df[(vendas_df["Item"] == produto_retirado) & (vendas_df["Tipo"] == "Produto")]["Quantidade"].sum() if not vendas_df.empty and "Item" in vendas_df.columns else 0
                        consumos_totais_item = consumo_interno_df[consumo_interno_df["Item"] == produto_retirado]["Quantidade"].sum() if not consumo_interno_df.empty and "Item" in consumo_interno_df.columns else 0
        ...
        with col_staff2:
            ...
            if not consumo_interno_df.empty and "Responsavel" in consumo_interno_df.columns:
                ...
                if st.button("❌ Cancelar Item Selecionado", use_container_width=True, key="btn_delete_staff_item"):
                    ...
                    texto_rastro = f"Profissional: {item_deletado['Responsavel']} | Item/Tipo: {item_deletado['Item']} | Valor Estornado: R$ {item_deletado['Valor Total']}"
        ...
        if st.session_state["perfil"] == "admin":
            if st.button("⚠️ Limpar Extrato da Semana (Apenas Admin)", key="btn_clear_staff_week"):
                ...
                st.rerun()
    with tab_rel3:
        if not v_filtradas.empty and "Data" in v_filtradas.columns: st.line_chart(v_filtradas.groupby("Data")["Valor Total"].sum())
...
# ---------------- MÓDULO 11: EMISSÃO DE RELATÓRIOS OFICIAIS EM PDF ----------------
    ...
    with st.container(border=True):
        ...
        if st.button("🧱 Processar e Compilar Relatório PDF", type="primary", use_container_width=True):
            vendas_df["Data"] = vendas_df["Data"].astype(str)
            v_filtradas = vendas_df[(vendas_df["Data"] >= data_inicio) & (vendas_df["Data"] <= data_fim)] if not vendas_df.empty else pd.DataFrame()
            gastos_df["Data"] = gastos_df["Data"].astype(str)
            g_filtrados = gastos_df[(gastos_df["Data"] >= data_inicio) & (gastos_df["Data"] <= data_fim)] if not gastos_df.empty else pd.DataFrame()
            fechamentos_df["Data"] = fechamentos_df["Data"].astype(str)
            f_filtrados = fechamentos_df[(fechamentos_df["Data"] >= data_inicio) & (fechamentos_df["Data"] <= data_fim)] if not fechamentos_df.empty else pd.DataFrame()

            faturamento_tot = pd.to_numeric(v_filtradas["Valor Total"], errors='coerce').sum() if not v_filtradas.empty else 0.0
            gastos_tot = pd.to_numeric(g_filtrados["Valor (R$)"], errors='coerce').sum() if not g_filtrados.empty else 0.0
            ...
            pdf.set_font("Arial", "", 10)
            servicos_tot = v_filtradas[v_filtradas["Tipo"] == "Serviço"]["Valor Total"].sum() if not v_filtradas.empty else 0.0
            produtos_tot = v_filtradas[v_filtradas["Tipo"] == "Produto"]["Valor Total"].sum() if not v_filtradas.empty else 0.0
            ...
            for meio in ["Pix", "Dinheiro", "Cartão de Crédito", "Cartão de Débito"]:
                val_meio = v_filtradas[v_filtradas["Forma de Pagamento"] == meio]["Valor Total"].sum() if not v_filtradas.empty else 0.0
...
# ---------------- MÓDULO 12: TELA DE AUDITORIA EXCLUSIVA DO SUPER ADMIN ----------------
        ...
        with st.container(border=True):
            if not auditoria_df.empty:
                ...
                if st.button("🗑️ Limpar Todo o Histórico de Logs", type="secondary", use_container_width=True):
                    ...
                    time.sleep(1.0)
                    st.rerun()
            else: st.info("🟢 O sistema está 100% limpo! Nenhuma exclusão ou estorno foi realizado no período.")
    else: st.error("🚨 ÁREA RESTRITA: Apenas o Super Administrador Geral possui acesso.")
...
# ---------------- MÓDULO 14: CONFIGURAÇÕES ----------------
    ...
    if st.session_state["perfil"] == "admin":
        ...
        with st.container(border=True):
            st.subheader("💳 Clube de Assinaturas")
